...
<div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit: {{ $flower->name }}</h1>
    <form action="{{ route('admin.flowers.update', $flower) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        @csrf @method('PUT')
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Name *</label>
            <input type="text" name="name" value="{{ old('name', $flower->name) }}" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-pink-500 focus:ring-pink-500" required>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-pink-500 focus:ring-pink-500" required>{{ old('description', $flower->description) }}</textarea>
            @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Price *</label>
                <input type="number" name="price" value="{{ old('price', $flower->price) }}" step="0.01" min="0" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-pink-500 focus:ring-pink-500" required>
                @error('price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Stock *</label>
                <input type="number" name="stock" value="{{ old('stock', $flower->stock) }}" min="0" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-pink-500 focus:ring-pink-500" required>
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="available" value="1" {{ old('available', $flower->available) ? 'checked' : '' }} class="rounded text-pink-600 focus:ring-pink-500">
            <label class="text-sm text-gray-700">Available for purchase</label>
        </div>
        <div class="flex gap-2 pt-2">
            <button type="submit" class="bg-pink-600 text-white px-6 py-2 rounded hover:bg-pink-700 text-sm font-semibold">Update Flower</button>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <div class="relative group">
                ...
                <form action="{{ route('admin.flowers.destroyImage', $img) }}" method="POST" class="absolute top-0 right-0">
                    @csrf @method('DELETE')
                    <button type="submit" class="bg-red-600 text-white rounded-full w-5 h-5 text-xs flex items-center justify-center opacity-0 group-hover:opacity-100">&times;</button>
                </form>
            </div>
            @endforeach
        </div>
